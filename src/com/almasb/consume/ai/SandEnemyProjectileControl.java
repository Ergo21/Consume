...
public class SandEnemyProjectileControl extends AbstractControl {
	...
	public SandEnemyProjectileControl(Entity player, boolean diag) {
		this.player = player;
		diagonal = diag;
		created = 0;
	}
		...
		if (now - created > TimerManager.toNanos(Config.SAND_DELAY)) {
			if(!positioned){
				if(player == null){
					...
					if (facingRight) {
						p = p.add(player.getWidth() + 50, (player.getHeight() - entity.getHeight())/2);
					} else {
						p = p.add(-entity.getWidth() - 50, (player.getHeight() - entity.getHeight())/2);
					}
